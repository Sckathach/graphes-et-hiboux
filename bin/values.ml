...
let backward v =
  ...
  build_topo v;
  v.grad <- 1.0;
  List.iter (fun node -> node._backward ()) (List.rev !topo)

let _ =
  let a = make_value ~label:"a" 2.0 in
  let b = make_value ~label:"b" 3.0 in
  let c = add a (`Value b) in
  let d = mul c (`Float 2.0) in
  let e = exp d in
  let f = tanh e in
  backward f;
  printf "%s\n" (string_of_value f);
  printf "Gradient of a: %f\n" a.grad;
  printf "Gradient of b: %f\n" b.grad
